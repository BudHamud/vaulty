...
const AnimeForm = ({ onSubmit, initialData = null }) => {
  ...
  const [form, setForm] = useState({
    title: "",
    status: "Pendiente",
    description: initialData?.description || initialData?.note || "", // Migración en caliente
    currentEp: 0,
    totalEp: "",
    imgUrl: "",
    startDate: today,
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      if (form.title.length > 3 && !initialData) {
        setLoading(true);
        const results = await searchAnimeOnMAL(form.title);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="relative">
        <input
          className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-400 font-bold text-slate-700"
          placeholder="Nombre del anime..."
          value={form.title}
          onChange={(e) => setForm({ ...form, title: e.target.value })}
        />
        {loading && <span className="absolute right-4 top-4 animate-spin">⏳</span>}
            ...
            {suggestions.map((s, i) => (
              <div key={i} onClick={() => selectSuggestion(s)} className="flex items-center gap-3 p-3 hover:bg-slate-50 cursor-pointer border-b border-slate-50 last:border-none">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{s.releaseYear} • Score: {s.score || "N/A"}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          <label className={`text-[10px] font-black uppercase ml-1 ${form.status !== "Terminado" ? "text-slate-200" : "text-slate-400"}`}>Finalización</label>
      ...
      <div className="flex gap-3 pt-2">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-2xl border-2 transition-all font-black text-[10px] uppercase tracking-tighter ${
            form.isFocus 
